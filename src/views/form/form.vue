<template>
  <section class="form-section">
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
      <el-form-item label="填写申请人" prop="person">
...
<!--      </el-form-item>-->
      ...
      <el-form-item>
        ...
        <el-button type="primary" @click="selectApprovel">选择审批人</el-button>
        <el-button @click="resetForm('ruleForm')">重置</el-button>
      </el-form-item>
    </el-form>

    <el-dialog title="请选择审批人" :visible="selectApprovalPersonVisible" size="tiny" :before-close="handleDialogClose">
      <select-bar :routerData = "item" v-for="item in items"></select-bar>
      <div slot="footer" class="dialog-footer">
        <el-button @click="selectApprovalPersonVisible = false">取消</el-button>
        <el-button type="primary" @click="selectApprovalPersonVisible = false">开启工单</el-button>
      </div>
    </el-dialog>
  </section>
</template>
<script>
  ...
  export default {
      ...
      return {
        ...
        selectApprovalPersonVisible: false,
        items:[],
        forbidden: true,
        redStar: false,
        ruleForm: {
...
//          关闭重复提醒功能，将表单验证中的相应规则，替换为空，并移除红色星号。不能删除相关规则，否则无法重新验证了，残留表单验证信息。
      ...
      selectApprovel(){
          this.selectApprovalPersonVisible = true;
      ...
      submitForm(formName) {
        ...
        const parameterD = {}
        parameterD.applyUserName = this.ruleForm.person
        parameterD.applyContentDetail = JSON.stringify(this.ruleForm)
        parameterD.systemType = 'CD'
            // let para = Object.assign({}, this.ruleForm);
        console.log(JSON.stringify(parameterD));
        addTask(parameterD).then(response => {
          console.log(response)
          this.$message({
            type: 'success',
            message: '创建成功'
      ...
      handleDialogClose(){
        this.selectApprovalPersonVisible =false
      }
    }
  }
</script>
