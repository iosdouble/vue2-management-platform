  ...
  <section>
        ...
        <el-table
          ...
          <el-table-column label="状态" class-name="status-col" width="100">
            <template slot-scope="{row}">
              <el-tag :type="row.processIsuseable | statusFilter">
                {{ row.processIsuseable==1 ? "使用中" :"未使用" }}
          ...
          <el-table-column label="操作">
            <template slot-scope="{row}">
              <el-button v-if="row.processIsuseable == 0" type="primary" size="small" @click="enableProcess(row)">启用流程</el-button>
              <el-button v-else type="danger" size="small" @click="disAbleProcess(row)">禁用流程</el-button>
    ...
    <el-dialog title="查看流程详情" :visible="flowDetialVisible" size="tiny" :before-close="handleDialogClose">
      <el-steps :active="orderStatus" >
        <el-step :title="item.nodeName" :description="item.nodeName" v-for="item in stepDatas"></el-step>
      </el-steps>
    </el-dialog>
  </section>
</template>
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      getList(){
          this.$http.get('/apis/getFlow').then((response) => {
      ...
      showFlowDetail(row){
        ...
        console.log(row)
        this.$http.get("/apis/getProcessById?pid="+row.processId).then((response)=>{
            console.log(response)
